...
describe TaskMailer, type: :mailer do
  ...
  describe '#creation_email' do
    ...
    it '想定通りのメールが生成されている' do
      ...
      # text
      expect(text_body).to eq('以下のタスクを作成しました')
      expect(text_body).to eq('メイラーSpecを書く')
      expect(text_body).to eq('送信したメールの内容を確認します')
      
      # html
      expect(html_body).to eq('以下のタスクを作成しました')
      expect(html_body).to eq('メイラーSpecを書く')
      expect(html_body).to eq('送信したメールの内容を確認します')
    end
